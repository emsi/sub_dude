...
def navigation_buttons(
    ...
    back_col, forth = st.columns([0.1, 0.1])

    if back and back_col.button(back_label, key=f"back_{position}"):
        st.session_state["STATE"] = back
        st.experimental_rerun()

    if forward and forth.button(forward_label, key=f"forward_{position}"):
